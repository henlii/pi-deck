...
test("buildSessionNavigationSnapshot 附带 observationalMemory；om custom 不进消息列表", () => {
  const entries = [
    userEntry("u1", null, "root"),
    {
      type: "custom",
  ...
  assert.ok(snap.observationalMemory);
  assert.equal(snap.observationalMemory.counts.observationsActive, 1);
  assert.equal(snap.observationalMemory.observations[0].id, "aaaaaaaaaaaa");
  // type:custom 不得伪装成聊天气泡
  assert.equal(
    snap.context.messages.some((m) => m.customType === "om.observations.recorded"),
    false,
  );
  assert.ok(snap.context.entryIds.includes("u1"));
  assert.ok(snap.context.entryIds.includes("a1"));
  assert.equal(snap.context.entryIds.includes("om1"), false);
});

test("buildSessionNavigationSnapshot 附带 workspaceHistory；snapshot custom 不进消息列表", () => {
  const entries = [
    userEntry("u1", null, "root"),
    {
      type: "custom",
  ...
  const snap = buildSessionNavigationSnapshot(sm);
  assert.ok(snap.workspaceHistory);
  assert.equal(snap.workspaceHistory.hasData, true);
  assert.equal(snap.workspaceHistory.counts.total, 2);
  assert.equal(snap.workspaceHistory.counts.byKind.before, 1);
  assert.equal(snap.workspaceHistory.counts.byKind.after, 1);
  assert.deepEqual(snap.workspaceHistory.markers.map((m) => m.kind), ["before", "after"]);
  assert.equal(snap.workspaceHistory.markers[0].shortCommit, "abcdef0");
  // type:custom 不得伪装成聊天气泡
  assert.equal(
    snap.context.messages.some((m) => m.customType === "workspace-history.snapshot"),
    false,
  );
  assert.ok(snap.context.entryIds.includes("u1"));
  assert.ok(snap.context.entryIds.includes("a1"));
  assert.equal(snap.context.entryIds.includes("wh1"), false);
  assert.equal(snap.context.entryIds.includes("wh2"), false);
});
