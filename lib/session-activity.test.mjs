...
test("rpc-manager send 暴露 append_activity 且固定 customType（源码契约）", async () => {
  ...
  assert.match(source, /appendCustomEntry\(\s*PIDANCE_ACTIVITY_CUSTOM_TYPE/);
  assert.doesNotMatch(source, /appendCustomMessageEntry/);
  // 禁止调用方自定义 customType
