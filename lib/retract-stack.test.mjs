...
test("removeRetracted 移除目标及其子孙链记录，并行兄弟保留", () => {
  ...
  assert.deepEqual(after.map((r) => r.entryId), ["u1"]);
  ...
  assert.deepEqual(pa.map((r) => r.entryId), [], "u1 的子孙全部移出");
  const pa2 = removeRetracted(parallel, ps, "u2");
  assert.deepEqual(pa2.map((r) => r.entryId), ["u1"]);
});
