...
test("basename(cwd) 映射到 projects-memory", async () => {
  ...
  try {
    writeTree(agentRoot, {
      ...
      "projects-memory/pi-deck/MEMORY.md": `proj-a${DELIM}proj-b`,
    });
    const snap = readHermesMemory({
      agentRoot,
      cwd: "/somewhere/pi-deck",
      homedir: () => "/home/u",
    });
    assert.ok(snap.project);
    assert.equal(snap.project.name, "pi-deck");
    assert.equal(
      snap.project.dir,
      path.join(agentRoot, "projects-memory", "pi-deck"),
    );
    assert.deepEqual(
