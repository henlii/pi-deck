...
// ---------------------------------------------------------------------------
// P1-4：rpc-manager ↔ session-service 双向循环依赖消除
// 导航动作（select_leaf_exact / branch_from_assistant / create_session_from_leaf）
// 改为 AgentSessionWrapper 构造注入；未注入时降级抛明确错误。
// ---------------------------------------------------------------------------

test("P1-4 静态门禁：rpc-manager 不再 import session-service，导航动作经注入 seam", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");

  // 环消除：运行时 import 已删除（类型 import 亦不允许——NavigationActions 定义在本模块）
  assert.doesNotMatch(source, /from\s+["']\.\/session-service["']/);
  assert.doesNotMatch(source, /from\s+["']@\/lib\/session-service["']/);
  assert.doesNotMatch(source, /sessionService\./);

  // 注入 seam 契约存在
  assert.match(source, /type NavigationActions/);
  assert.match(source, /new AgentSessionWrapper\(inner, navigationActions\)/);
  assert.match(source, /navigationActionsOrThrow\(/);
});

test("wrapper：未注入 navigation actions 时三个导航命令降级抛明确错误", async (t) => {
  const { AgentSessionWrapper } = await jiti.import("./rpc-manager.ts");
  const stub = createActivityStub();
  const wrapper = trackWrapper(t, new AgentSessionWrapper(stub.inner));

  await assert.rejects(
    () => wrapper.send({ type: "select_leaf_exact", entryId: "e1" }),
    /select_leaf_exact is unavailable: navigation actions were not injected/,
  );
  await assert.rejects(
    () => wrapper.send({ type: "branch_from_assistant", assistantEntryId: "a1" }),
    /branch_from_assistant is unavailable: navigation actions were not injected/,
  );
  await assert.rejects(
    () => wrapper.send({ type: "create_session_from_leaf", entryId: "e1" }),
    /create_session_from_leaf is unavailable: navigation actions were not injected/,
  );
});

test("wrapper：注入 navigation actions 时三个命令转发到动作（trim + sessionId 语义保持）", async (t) => {
  const { AgentSessionWrapper } = await jiti.import("./rpc-manager.ts");
  const stub = createActivityStub();
  /** @type {Array<[string, string, string]>} */
  const calls = [];
  const actions = {
    selectLeafExact: async (sessionId, entryId) => {
      calls.push(["selectLeafExact", sessionId, entryId]);
      return { cancelled: false };
    },
    branchFromAssistant: async (sessionId, assistantEntryId) => {
      calls.push(["branchFromAssistant", sessionId, assistantEntryId]);
      return { cancelled: false };
    },
    createSessionFromLeaf: async (sessionId, entryId) => {
      calls.push(["createSessionFromLeaf", sessionId, entryId]);
      return { cancelled: false, newSessionId: "ns-1" };
    },
  };
  const wrapper = trackWrapper(t, new AgentSessionWrapper(stub.inner, actions));

  const r1 = await wrapper.send({ type: "select_leaf_exact", entryId: "  e1  " });
  const r2 = await wrapper.send({ type: "branch_from_assistant", assistantEntryId: " a1 " });
  const r3 = await wrapper.send({ type: "create_session_from_leaf", entryId: "e2" });

  assert.equal(r1.cancelled, false);
  assert.equal(r2.cancelled, false);
  assert.equal(r3.cancelled, false);
  assert.equal(r3.newSessionId, "ns-1");
  assert.deepEqual(calls, [
    ["selectLeafExact", "act-test-session", "e1"],
    ["branchFromAssistant", "act-test-session", "a1"],
    ["createSessionFromLeaf", "act-test-session", "e2"],
  ]);

  // 参数校验仍在 wrapper 侧：非法 entryId 不触达动作
  await assert.rejects(
    () => wrapper.send({ type: "select_leaf_exact", entryId: "  " }),
    /entryId is required/,
  );
