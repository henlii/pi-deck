...
test("startRpcSession 三个启动路径均注入 navigation actions，且代理到本 service 实例", async () => {
  ...
  // 动作代理到本 service 实例语义：wrapper 不存在时抛 "Session is not live"
  //（而非哑函数或全局单例），证明注入的正是 createSessionService 的落地实现。
  const navService = createSessionService({
    ...
    startRpcSession: async (_sessionId, _sessionFile, _cwd, _toolNames, navigationActions) => {
      await assert.rejects(
        () => navigationActions.selectLeafExact("ghost", "e1"),
        /Session is not live/,
      );
      await assert.rejects(
        () => navigationActions.branchFromAssistant("ghost", "a1"),
        /Session is not live/,
      );
      await assert.rejects(
        () => navigationActions.createSessionFromLeaf("ghost", "e1"),
        /Session is not live/,
      );
      return { session: fresh, realSessionId: "nav-fresh" };
    },
