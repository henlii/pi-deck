...
test("mapExtensionNotifyToActivity：workspace-history disabled 提示不持久化", async () => {
  const { mapExtensionNotifyToActivity } = await loadEvents();

  // 纯提示性 warning（workspace-history disabled 提示）：transient，不持久化。
  assert.equal(
    mapExtensionNotifyToActivity({
      message: "Workspace history is disabled for this directory: current directory is the user home folder. Open pi inside a project directory or set workspaceHistory.enabled to true.",
      notifyType: "warning",
      requestId: "r-wh",
    }),
    null,
  );
  // 前缀不匹配的同类 warning 仍持久化（仅匹配已知提示前缀）。
  assert.notEqual(
    mapExtensionNotifyToActivity({
      message: "Workspace history is enabled for this directory",
      notifyType: "warning",
      requestId: "r-wh2",
    }),
    null,
