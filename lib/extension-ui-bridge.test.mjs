...
test("FIFO：dialog → inline 后续入队，队首仍为 dialog", () => {
  const dialog = request("editor", { title: "编辑", id: "dialog" });
  const inline = request("input", { title: "输入", id: "inline" });
  let state = applyExtensionUiRequest(base, dialog).state;
  state = applyExtensionUiRequest(state, inline).state;
  assert.equal(state.dialog?.id, "dialog");
  assert.equal(state.inlineRequest, null);
  assert.deepEqual(state.blockingQueue.map((item) => item.id), ["dialog", "inline"]);
});

test("清队首推进：移除后自动投影下一项（inline→dialog）", () => {
  const a = request("confirm", { title: "A", id: "a" });
