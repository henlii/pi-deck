...
test("GET 投影不含原始 apiKey / 敏感 header 值", () => {
  withDir((dir) => {
    ...
    assert.ok(!("apiKey" in p), "投影中不得出现 apiKey 字段");
...
test("GET 未配置 apiKey 的 provider 返回 apiKeyConfigured=false", () => {
  withDir((dir) => {
    ...
    assert.ok(!("apiKey" in result.providers.plain));
