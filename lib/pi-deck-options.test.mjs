...
const { parseLaunchOptions } = require("../bin/pi-deck-options.js");
...
test("supports truthy PI_DECK_NO_OPEN values", () => {
  for (const value of ["1", "true", "TRUE", "yes", "on"]) {
    assert.equal(parseLaunchOptions([], { PI_DECK_NO_OPEN: value }).openBrowser, false);
  }
});

test("supports truthy PI_WEB_NO_OPEN values for compatibility", () => {
  for (const value of ["1", "true", "TRUE", "yes", "on"]) {
    assert.equal(parseLaunchOptions([], { PI_WEB_NO_OPEN: value }).openBrowser, false);
  }
});

test("does not disable browser opening for false PI_DECK_NO_OPEN values", () => {
  for (const value of ["0", "false", "off", ""]) {
    assert.equal(parseLaunchOptions([], { PI_DECK_NO_OPEN: value }).openBrowser, true);
  }
});
