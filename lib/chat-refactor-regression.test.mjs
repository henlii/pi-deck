 ...
 * 正向断言（兼容 RPC 保留、核心模块在位）对原始源码断言，防误删。
 */
...
test("ChatWindow.tsx 不再挂载 RetractedMessagesDock / OmPanel / WorkspaceHistoryPanel", async () => {
  ...
  assert.match(src, /<InlineExtensionCard/);
});
...
test("rpc-manager.ts 保留撤回/恢复兼容 RPC（allowlist 语义，防误删）", async () => {
  // P0 撤回/恢复的 UI（RetractedMessagesDock）已下线，但 retract_message /
  // restore_message / list_retracted 三个 RPC 仍作为兼容层保留：服务端实现
  // 只按 Pi 分支语义走 navigate_tree（移动 leaf，不产生物理删除），既有客户端/
  // 扩展可能仍调用。断言其「存在」而非「禁止」，防未来误删破坏兼容契约。
  const src = await readFile(srcUrl("lib/rpc-manager.ts"), "utf8");

  for (const cmd of ['case "retract_message"', 'case "restore_message"', 'case "list_retracted"']) {
    assert.ok(src.includes(cmd), `rpc-manager.ts 必须保留 ${cmd}`);
  }
});
