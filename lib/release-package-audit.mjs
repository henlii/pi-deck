...
export const FORBIDDEN_PATH_PATTERNS = [
  ...
  { kind: "exact", value: ".next/required-server-files.json" },
  ...
  { kind: "exact", value: "public" },
  { kind: "prefix", value: "public/" },
];
...
/**
 * 扫描文本中的敏感内容。
 * deck.namixinxi.cn 允许；不笼统拒绝 /tmp。
 *
 * @param {string} text
 * @param {{ repoRoot?: string | null, homeDir?: string | null }} [opts]
 * @returns {string[]}
 */
export function scanSensitiveContent(text, opts = {}) {
  ...
  if (
    /(?:api[_-]?key|secret[_-]?key|access[_-]?token|auth[_-]?token|private[_-]?key)\s*[:=]\s*['"`][^'"`\s]{8,}/i.test(
      text,
    )
  ) {
  ...
  const repoRoot = opts.repoRoot ? normalizePackPath(opts.repoRoot).replace(/\/$/, "") : null;
  if (repoRoot && repoRoot.length >= 4) {
    const escaped = repoRoot.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    if (new RegExp(escaped, "i").test(text.replace(/\\/g, "/"))) {
      reasons.push(`敏感内容: 仓库绝对路径 ${repoRoot}`);
    }
  }

  const homeDir = opts.homeDir ? normalizePackPath(opts.homeDir).replace(/\/$/, "") : null;
  if (homeDir && homeDir.length >= 2 && homeDir !== "/" && homeDir !== repoRoot) {
...
export function auditReleasePackage(input) {
  ...
  if (bin && typeof bin === "object") {
    if (!Object.prototype.hasOwnProperty.call(bin, "pi-deck")) {
      violations.push({ path: "package.json#bin", reason: "缺少 bin.pi-deck" });
    }
  ...
  const hasPiDeckBin =
    pathSet.has("bin/pi-deck.js") ||
    (bin && typeof bin["pi-deck"] === "string" && pathSet.has(normalizePackPath(bin["pi-deck"])));
  if (!hasPiDeckBin) {
    violations.push({ path: "bin/pi-deck.js", reason: "包内缺少 pi-deck CLI 入口" });
  }
...
export function auditReleaseTgz(input) {
  ...
  if (pkg.bin && typeof pkg.bin === "object" && !Array.isArray(pkg.bin)) {
    ...
    bin = { [name.includes("/") ? name.split("/").pop() : name || "pi-deck"]: pkg.bin };
  }
