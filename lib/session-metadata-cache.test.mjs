...
const HEADER = JSON.stringify({
  type: "session", version: 3, id: "abc12345", timestamp: "2026-08-01T00:00:00.000Z",
  cwd: "/root", parentSession: "/root/parent.jsonl",
});
...
test("scanSessionFileFast 提取列表元数据", async () => {
  ...
  try {
    const file = writeSession(sessionsRoot, "--root--", "a.jsonl", [
      ...
      JSON.stringify({ type: "model_change", id: "m1", parentId: null, provider: "x", modelId: "y", timestamp: "2026-08-01T00:00:01.000Z" }),
      JSON.stringify({ type: "message", id: "u1", parentId: null, timestamp: "2026-08-01T00:00:02.000Z", message: { role: "user", content: "你好" } }),
      JSON.stringify({ type: "message", id: "u2", parentId: "u1", timestamp: "2026-08-01T00:00:03.000Z", message: { role: "user", content: [{ type: "text", text: "第二条" }] } }),
      JSON.stringify({ type: "session_info", id: "i2", parentId: null, name: "新标题" }),
      JSON.stringify({ type: "message", id: "a1", parentId: "u2", timestamp: "2026-08-01T00:00:04.000Z", message: { role: "assistant", content: "回复" } }),
      "坏行",
      JSON.stringify({ type: "message", id: "r1", parentId: "a1", timestamp: 1723000000000, message: { role: "toolResult", toolCallId: "t1", content: [{ type: "toolResult", toolName: "bash", content: "out" }] } }),
    ]);
    ...
    // modified = 最后 user/assistant 消息活动时间（toolResult 不计入，SDK 语义）
    assert.equal(info.modified, "2026-08-01T00:00:04.000Z");
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
...
test("缓存读写：写后读回一致；损坏文件返回 null", async () => {
  ...
  try {
    ...
    const sessions = new Map([
      ["/s/a.jsonl", { m: 111, s: 222, i: { id: "x1", cwd: "/root", created: "c", modified: "m", messageCount: 1, firstMessage: "f", name: "n" } }],
    ]);
    const discovery = new Map([
      ["/s/a.jsonl", { m: 111, s: 222, c: [{ path: "/s/a/child.jsonl", id: "c1", cwd: "/root", timestamp: "t", parentSessionId: "x1", runId: "r1", runIndex: 0 }] }],
    ]);
    ...
    // 版本不符：忽略
    writeFileSync(cacheFile, JSON.stringify({ version: 99, sessions: {} }), "utf8");
    assert.equal(loadSessionMetadataCache(cacheFile), null);
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
