...
} from "./session-fulltext-search.ts";
...
test("A2: JSONL 扫描命中与字节上限", async () => {
  const dir = mkdtempSync(join(tmpdir(), "pi-deck-search-"));
  try {
...
test("A2: resolveHermesSessionsDbPath 默认与空文件拒绝", () => {
  const dir = mkdtempSync(join(tmpdir(), "pi-deck-hermes-"));
  try {
...
test("A2: FTS 优先返回 source=fts；DB 失败降级 jsonl", async () => {
  ...
  // DB 打开失败 → JSONL 降级
  const dir = mkdtempSync(join(tmpdir(), "pi-deck-fallback-"));
  try {
