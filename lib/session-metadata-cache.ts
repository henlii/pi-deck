...
export async function scanSessionFiles(sessionRoot = join(getAgentDir(), "sessions")): Promise<SessionFileStat[]> {
  let dirs: string[] = [];
  try {
    const entries = await readdir(sessionRoot, { withFileTypes: true });
    dirs = entries.filter((e) => e.isDirectory()).map((e) => join(sessionRoot, e.name));
  } catch {
    return [];
  }
  const files: string[] = [];
...
export function loadSessionMetadataCache(filePath: string = cacheFilePath()): SessionMetadataCacheFile | null {
  try {
    ...
    return {
      ...
      discovery: parsed.discovery && typeof parsed.discovery === "object" ? parsed.discovery : {},
    };
  } catch {
    return null;
  }
...
function isMessageWithContent(message: unknown): message is { role?: unknown; content?: unknown; timestamp?: unknown } {
  return typeof message === "object" && message !== null && typeof (message as { role?: unknown }).role === "string" && "content" in (message as object);
}
 ...
 */
export async function scanSessionFileFast(filePath: string, st: { mtimeMs: number; size: number }): Promise<CachedSessionInfo | null> {
  try {
    ...
    for await (const line of rl) {
      ...
      if (entry.type === "session_info") {
        ...
        if (typeof infoName === "string" && infoName.trim()) name = infoName.trim();
        continue;
      }
...
export async function scanSessionFilesWithConcurrency(files: string[], concurrency = SCAN_CONCURRENCY): Promise<SessionFileStat[]> {
