...
test("写入临时 jsonl 后可被 SessionManager.open 再打开", async () => {
  const { buildSessionBranchJsonl, exportSessionFileToJsonl } = await load();
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-deck-export-"));
  try {
    // 用真实 SessionManager 建一个持久化会话并分叉
    ...
    const reA = SessionManager.open(outA);
    const reB = SessionManager.open(outB);
    assert.deepEqual(
    ...
    // 纯函数路径与文件路径一致（忽略 timestamp）
    const pure = buildSessionBranchJsonl(SessionManager.open(sessionFile), {
      leafId: leafA,
...
test("路由 format 未知 400、jsonl 非法 leaf 400、缺失会话 404、成功响应头与只读", async () => {
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-deck-export-route-"));
  let sessionId = "";
  try {
    const sm = SessionManager.create(dir, dir);
    const sessionFile = sm.getSessionFile();
