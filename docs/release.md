...
本文描述 **@henlii/pi-deck** 的正式发版流程。**不表示任何版本已发布**；任一步失败都不得宣称成功。

发布目标：

- npm 包：`@henlii/pi-deck@0.1.0`（及后续版本）
- GitHub Release：`henlii/pi-deck` 的 `v0.1.0`（及后续 tag）

Pi Deck 源自 [agegr/pi-web](https://github.com/agegr/pi-web)，底层仍兼容 [badlogic/pi-mono](https://github.com/badlogic/pi-mono) 的会话语义。发版前核对上游变更，保持会话文件与运行时兼容。
...
6. 公开安装入口 CLI **仅** `pi-deck`；**不得**注册 `pi-web`（该命令归上游）。产品默认端口 **31415**；**30141** 留给上游 pi-web，禁止操作。
...
```bash
git worktree add /tmp/pi-deck-release-v0.1.0 v0.1.0
cd /tmp/pi-deck-release-v0.1.0
npm ci
```
...
# 得到例如：henlii-pi-deck-0.1.0.tgz

npm run release:audit:tgz -- henlii-pi-deck-0.1.0.tgz
# 或：node scripts/audit-release-package.mjs --tgz henlii-pi-deck-0.1.0.tgz
```
...
```bash
sha256sum henlii-pi-deck-0.1.0.tgz | tee henlii-pi-deck-0.1.0.tgz.sha256
```
...
```bash
mkdir -p /tmp/pi-deck-smoke && cd /tmp/pi-deck-smoke
npm init -y
npm install /tmp/pi-deck-release-v0.1.0/henlii-pi-deck-0.1.0.tgz --omit=dev
npx pi-deck --help || true
# 或短时启动：npx pi-deck --no-open -p 31415
# 确认监听 31415，且命令名为 pi-deck（无 pi-web）
```
...
npm whoami --registry https://registry.npmjs.org/   # 确认有权发布 @henlii/pi-deck

npm publish /path/to/henlii-pi-deck-0.1.0.tgz \
  --access public \
...
```bash
npm view @henlii/pi-deck@0.1.0 version --registry https://registry.npmjs.org/
```
...
gh release create v0.1.0 \
  ...
  henlii-pi-deck-0.1.0.tgz \
  henlii-pi-deck-0.1.0.tgz.sha256
```
...
```bash
gh release view v0.1.0 --repo henlii/pi-deck
npm view @henlii/pi-deck@0.1.0 version --registry https://registry.npmjs.org/
```
